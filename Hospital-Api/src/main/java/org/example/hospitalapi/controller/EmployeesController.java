...
import org.example.hospitalapi.dtos.UpdateEmployeeDepartmentRequest;
...
public class EmployeesController {
  ...
  public ResponseEntity<EmployeeResponse> updateEmployeeStatus (@PathVariable Long id ,@RequestBody @Valid UpdateEmployeeStatusRequest updateEmployeeStatusRequest){
    Optional<EmployeeResponse> employeeResponse = employeeService.updateEmployeeStatus(id, updateEmployeeStatusRequest);
    return employeeResponse.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
  }
  @PatchMapping("/{id}/updateEmployeeDepartment")
  public ResponseEntity<EmployeeResponse> updateEmployeeStatus (@PathVariable Long id ,@RequestBody @Valid UpdateEmployeeDepartmentRequest updateEmployeeDepartmentRequest){
    Optional<EmployeeResponse> employeeResponse = employeeService.updateEmployeeDepartment(id, updateEmployeeDepartmentRequest);
    return employeeResponse.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
  }
